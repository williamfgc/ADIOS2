...
BPVizWriter::BPVizWriter(IO &io, const std::string &name, const Mode mode,
                         MPI_Comm mpiComm)
: Engine("BPVizWriter", io, name, mode, mpiComm),
  m_BP3Serializer(mpiComm, m_DebugMode),
  m_FileDataManager(mpiComm, m_DebugMode),
  m_FileMetadataManager(mpiComm, m_DebugMode)
{
    m_EndMessage = " in call to IO Open BPVizWriter " + m_Name + "\n";
    Init();
}

BPVizWriter::~BPVizWriter() = default;

StepStatus BPVizWriter::BeginStep(StepMode mode, const float timeoutSeconds)
{
...
void BPVizWriter::Close(const int transportIndex)
{
    if (m_BP3Serializer.m_DeferredVariables.size() > 0)
    {
        PerformPuts();
...
#define declare_type(T)                                                        \
    void BPVizWriter::DoPutSync(Variable<T> &variable, const T *values)        \
    {                                                                          \
        PutSyncCommon(variable, values);                                       \
    }                                                                          \
    void BPVizWriter::DoPutDeferred(Variable<T> &variable, const T *values)    \
    {                                                                          \
        PutDeferredCommon(variable, values);                                   \
    }                                                                          \
    void BPVizWriter::DoPutDeferred(Variable<T> &, const T &value) {}
ADIOS2_FOREACH_TYPE_1ARG(declare_type)
#undef declare_type

void BPVizWriter::InitParameters()
{
    m_BP3Serializer.InitParameters(m_IO.m_Parameters);
}

void BPVizWriter::InitTransports()
{
...
void BPVizWriter::WriteProfilingJSONFile()
{
...
void BPVizWriter::WriteCollectiveMetadataFile()
{
